...
int aes_setkey(aes_context *ctx,   // AES context provided by our caller
               int mode,           // GCM_ENCRYPT or GCM_DECRYPT flag
               ...
               size_t keysize)     // key length in bytes
{
    ...
    if (mode == GCM_DECRYPT) // expand our key for encryption or decryption
                                     ...
                                     // 128, 192 or 256-bit keys respectively)
{
    ...
    if ((ret = aes_setkey(&ctx->aes_ctx, GCM_ENCRYPT, key, keysize)) != 0)
        return (ret);
              ...
              size_t add_len)     // length of additional AEAD data (bytes)
{
    ...
    ctx->aes_ctx.mode = GCM_ENCRYPT; // GCM *always* runs AES in ENCRYPTION mode
...
{                         /*
    ...
    if ((ret = gcm_start(ctx, mode, iv, iv_len, add, add_len)) != 0)
    {
        return (ret);
    }
    ...
    size_t tag_len)       // byte length of the tag <= 16
{
    ...
    */
    if ((ret = gcm_crypt_and_tag(ctx, GCM_DECRYPT, iv, iv_len, add, add_len,
                                 input, output, length, check_tag, tag_len)) != 0)
    {
        return (ret);
    }
...
int aes_gcm_encrypt(uint8_t *output, const uint8_t *input, const size_t input_length, const uint8_t *key, const size_t key_len, const uint8_t *iv, const size_t iv_len, const uint8_t *aad, const size_t aad_len, uint8_t *tag, const size_t tag_len)
{
    ...
    if ((ret = gcm_setkey(&ctx, key, key_len)) != 0)
    {
        return (ret);
    }

    if ((ret = gcm_crypt_and_tag(&ctx, GCM_ENCRYPT, iv, iv_len, aad, aad_len,
                                 input, output, input_length, tag, tag_len)) != 0)
    {
        return (ret);
    }
...
int aes_gcm_decrypt(uint8_t *output, const uint8_t *input, const size_t input_length, const uint8_t *key, const size_t key_len, const uint8_t *iv, const size_t iv_len, const uint8_t *aad, const size_t aad_len, uint8_t *tag, const size_t tag_len)
{
    ...
    if ((ret = gcm_crypt_and_tag(&ctx, GCM_DECRYPT, iv, iv_len, aad, aad_len,
                                 input, output, input_length, tag, tag_len)) != 0)
    {
        return (ret);
    }
